...
df = pd.read_csv('/Users/jonas/Downloads/goog3_media_type.csv')
df_nums = df.loc[:, df.columns != 'media']
...
# getting the maxs
df['max'] = dd.max(axis=1)
df['max_boost'] = dd.loc[:, dd.columns != 'media'].idxmax(axis=1)
m = df[['media', 'maxbid', 'max_boost', 'max']]
...
for media in df['media'].unique():
    ...
    ax2 = ax1.twinx()
    ax2.plot('maxbid', 'max', data=m, color='blue', alpha=0.5, linewidth=3.0, label = 'max_ecpm')
    ax2.grid()
